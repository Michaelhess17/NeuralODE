...
for subject in range(5, 16):
    model = eqx.tree_deserialise_leaves(f"outputs/vdp_model_subject_{subject}.eqx", model)
    new_ts = jnp.linspace(0, 120, 12000)
    seeding_steps = 100
    subject_ids = jnp.repeat(jnp.arange(20), 5)[25:-20]
    train_data = data_tde[subject_ids == subject]
    out = jax.vmap(solve_sde, in_axes=(None, None, 0, None))(model, new_ts, train_data[:, 0:seeding_steps, :], diffusion)

    jnp.save(f"outputs/gen_vdp_data_subject_{subject}.npy", out)
